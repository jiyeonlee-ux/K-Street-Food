document.addEventListener("DOMContentLoaded", () => {
  ...
  // Navigation Links
  const tornadoLink = document.querySelector(".i-tornado a");
  const bungeoppangLink = document.querySelector(".i-bungeoppang a");
  const tteokLink = document.querySelector(".i-tteok a");
  const hotdogLink = document.querySelector(".i-hotdog a");
  const bbungtigiLink = document.querySelector(".i-bbungtigi a");
  ...
  let lastActivePage = cardListView;
  ...
  const showPage = (targetPage) => {
    ...
    if (targetPage !== detailView) {
      lastActivePage = targetPage;
    }

    // Ensure footer is always visible (style is now handled globally in CSS)
  ...
  if (tornadoLink && tornadoPage) {
    tornadoLink.addEventListener("click", (e) => {
      console.log("Tornado link clicked");
      e.preventDefault();
  ...
  if (bungeoppangLink && cardListView) {
    bungeoppangLink.addEventListener("click", (e) => {
      console.log("Bungeoppang link clicked");
      e.preventDefault();
      showPage(cardListView);
    });
  }
  ...
  if (tteokLink && tteokPage) {
    tteokLink.addEventListener("click", (e) => {
      console.log("Tteok link clicked");
      e.preventDefault();
      showPage(tteokPage);
    });
  }
  ...
  if (hotdogLink && hotdogPage) {
    hotdogLink.addEventListener("click", (e) => {
      console.log("Hotdog link clicked");
      e.preventDefault();
      showPage(hotdogPage);
    });
  }
  ...
  if (bbungtigiLink && grainPage) {
    bbungtigiLink.addEventListener("click", (e) => {
      console.log("Bbungtigi link clicked");
      e.preventDefault();
      showPage(grainPage);
    });
  }
  ...
  const fullFoodData = {
    ...
    "tornado-cheese": {
      ...
      desc: `
          ...
          <h3>The addiction factor</h3>
          <p>
            Cheese powder clings to every crevice – the fine dust coating means maximum flavor per bite without overwhelming. Unlike melted cheese, the powder lets you taste the crispy potato while adding savory-tangy depth. Each spiral delivers consistent cheese hit – your brain gets hooked on the salty-savory rhythm. The powder also slightly caramelizes on the hot potato, creating tiny flavor bombs in the crispy edges.
          </p>

          <h3>Visual pop</h3>
          <p>
            That golden potato dusted in orange-yellow cheese powder – instantly recognizable from across the street. The spiral shape shows off the even coating, looking like a cheese-flavored spring. When vendors shake the fresh-fried potato in the seasoning bag, the powder clouds up like a cheese storm, then settles into every ridge. It's less dramatic than melted cheese pulls, but the uniform golden-orange coating has its own hypnotic appeal.
          </p>
    ...
    "hotdog-original": {
      ...
      desc: `
          ...
          <h3>The addiction factor</h3>
          <p>
            The sugar coating is the plot twist – it caramelizes slightly on the hot surface, creating a sweet crust that contrasts with savory hot dog and tangy condiments. Your brain can't decide if it's dessert or dinner, so you just keep eating. The ultra-crunchy panko stays crispy way longer than American corn dog batter – it's structurally superior.
          </p>

          <h3>Visual pop</h3>
          <p>
            That golden panko armor studded with potato cubes (gamja hot dog) or squid ink black coating – it's armor plating for street food. When you bite into cheese versions, the mozzarella stretch is legendary. Vendors dust it with sugar right in front of you for maximum drama.
          </p>
    ...
    "hotdog-potato": {
      ...
      desc: `
          ...
          <h3>The addiction factor</h3>
          <p>
            Each bite delivers three textures: crunchy potato exterior → crispy panko layer → juicy hot dog (or stretchy cheese) core. The potato cubes get extra crispy in the fryer while staying fluffy inside – they're basically gourmet tater tots welded to your corn dog. The sugar dusting caramelizes on the hot potato, creating sweet-salty perfection that your brain wasn't ready for.
          </p>
    ...
    "hotdog-cheese": {
      ...
      desc: `
          ...
          <h3>Visual pop</h3>
          <p>
            The cheese pull moment is social media gold – that long, elastic mozzarella string between your mouth and the stick. The golden panko armor contrasts with white cheese oozing out. Vendors know the power of the pull, often demonstrating it before handing it over. It's performance art you can eat.
          </p>
    ...
    "grain-icecream": {
      ...
      desc: `
          ...
          <p>
            Bbungtigi (뻥튀기) is Korea's OG puffed snack – imagine rice cakes exploded into crispy clouds. Your grandparents bought it from street vendors with wooden carts. Now it's hijacked by Gen Z as an ice cream vessel, turning childhood memories into Instagram content.
          </p>

          <h3>The genius engineering</h3>
          <p>
            Unlike soggy waffle cones, bbungtigi stays crunchy for 10+ minutes thanks to its porous structure that doesn't absorb moisture quickly. Each bite delivers alternating waves of cold creaminess and crackling rice puffs – it's textural ASMR you can eat.
          </p>
